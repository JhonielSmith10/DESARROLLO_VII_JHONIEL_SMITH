...
// Tu código aquí
function estudiantesTutoria($estudiantes) {
...
$tutorados = estudiantesTutoria($estudiantes);
$honores = estudiantesDeHonor($estudiantes);
echo "</br>Estudiantes que necesitan tutoría:\n";
foreach ($tutorados as $tutorado) {
    echo "- {$tutorado['nombre']} (Promedio: {$tutorado['promedio']})\n";
}
echo "</br>Estudiantes de honor:\n";
foreach ($honores as $honor) {
    echo "- {$honor['nombre']} (Promedio: {$honor['promedio']})\n";
}
?>