...
function contarCiudadesPorInicial($arr, $inicial) {
    ...
    foreach ($arr as $ciudad) {
        if (stripos($ciudad, $inicial) === 0) {
            $contador++;
            $ciudades_inicial[] = $ciudad; 
        }
    }
    return [$contador, $ciudades_inicial];
}

$letra = 'N';
list($numCiudades, $ciudadesConInicial) = contarCiudadesPorInicial($ciudades, $letra);

echo "</br>Número de ciudades que comienzan con la letra '$letra': $numCiudades\n";
echo "</br>Ciudades que comienzan con la letra '$letra':\n";
foreach ($ciudadesConInicial as $ciudad) {
    echo "- $ciudad\n";
}
?>