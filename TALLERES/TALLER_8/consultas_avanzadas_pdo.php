...
try {
    ...
    echo "<h3>Promedio de publicaciones por usuario: " . $row['promedio_publicaciones'] . "</h3>";

    // 7. Encontrar la publicación más reciente de cada usuario
    $sql = "SELECT u.nombre, ANY_VALUE(p.titulo) as titulo, MAX(p.fecha_publicacion) as ultima_publicacion 
            FROM publicaciones p 
            INNER JOIN usuarios u ON p.usuario_id = u.id 
            GROUP BY u.id";
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        echo "Usuario: " . $row['nombre'] . ", Título: " . $row['titulo'] . ", Última Publicación: " . $row['ultima_publicacion'] . "<br>";
    }
